...
io.on('connection', socket => {
  ...
  socket.on('new_user', (name: string) => {
    const newUser = user.push({ accessId: socket.id, userId, userName: name, messages: new MessageLinkedList(), connectedUserNames: [{ name, userId }] });
