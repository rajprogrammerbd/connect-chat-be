...
if (cluster.isPrimary) {
  ...
  cluster.on('online', (worker) => {
    consola.success(`Worker ID is ${worker.id} and PID is ${worker.process.pid}`);
  });

  cluster.on('exit', (worker) => {
    consola.warn(`Worker ID ${worker.id} and PID is ${worker.process.pid} is offline`);
  ...
  consola.success(`Server is running with worker PID ${cluster.worker?.process.pid}`)
