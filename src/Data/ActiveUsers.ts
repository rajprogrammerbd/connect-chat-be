...
class ActiveUsers extends ActiveChatsHash {
    ...
    removeUser(chatId: string, userId: string) {
        ...
        if (user) {
            if (user.value?.isAdmin) {
                ...
                if (this.head?.value?.userId === userId && this.head.value.chatId === chatId) {
                ...
                while (current) {
                    if (current.next?.value?.userId === userId && current.next.value.chatId === chatId) {
    ...
    startTyping(chatId: string, userId: string) {
        ...
        const res = this.pushTypingList({ chatId, userId: userId, userName: userName, type: Msg_Types.started_chat, message: `${userName} is typing`, timestamp: new Date().toISOString()  });
    ...
    protected sendMessage(chatId: string, userId: string, userName: string, type: Msg_Type, msg?: string): boolean {
    ...
    adminChatUser(name: string): User | null {
        ...
        if (!this.userCount) {
            ...
            this.sendMessage(chatId, userId, newUser.value?.userName as string, Msg_Types.started_chat);
